...
if($f1 == 0)
{
	...
	`perl scripts/ortho_to_phonetic_tam.pl lists/trans_word lists/phone_list phn`;
	$count = scalar(split("\n",cat("phn")));
	$start = 2;
	...
	while ($start <= $count)
	{
		...
		else
		{
			...
			if ($phn eq "u")
			{
				...
				if(($start == $count) || ($phn2 eq "SIL") || ($flg_1 != 0 && $c1 == $count) || ($flg_1 != 0 && $phn2 eq "SIL") || ($phn1 eq "k" && $phn2 eq "k" && $c0 != 1))
				{
			...
			{
				if($phn0 eq "c" || $phn1 eq "c" || $c1 == $count || $phn2 eq "SIL")
				{
					print out_word_handle $phn;
				}
				elsif ($phn0 eq "nj")
